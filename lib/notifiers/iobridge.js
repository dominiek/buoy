...
IobridgeNotifier.prototype._setWidgets = function(hadFailures, numFailures) {
  ...
  for(var i=0; this.relaisIds.length>i; i++) {
    var value = 0;
    if(i == 0) {
      this._setRelais(this.relaisIds[i], hadFailures ? 1 : 0);
    }
    if(i == 1) {
      this._setRelais(this.relaisIds[i], hadFailures ? 0 : 1);
    }
  }
  if(this.lcdId) {
...
IobridgeNotifier.prototype._setRelais = function(id, value) {
