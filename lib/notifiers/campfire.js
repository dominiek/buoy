...
CampfireNotifier.prototype.enable = function(server) {
  console.log("Notifying results to Campfire chat room " + this.room + " on " + this.subdomain + ".campfirenow.com");
  server.on('environment-failed', function(report) {
    this._speak(utils.summarizeReport(report));
    this._paste(utils.summarizeErrors(report));
    //this._play('deeper');
  }.bind(this));
...
CampfireNotifier.prototype._paste = function(text) {
  this._getRoom(function(err, room) {
    room.paste(text);
  });
};
