...
Server.prototype.processReport = function(report) {
  ...
  if(report.passed) {
    console.log("[Buoy] Environment " + utils.humanReadableEnvironmentString(report.environment) + " OK (" + report.numPassed + " " + utils.pluralize("tests", report.numPassed) + " passed)");
    this.emit('environment-passed', report);
  } else {
    console.log("[Buoy] Environment " + utils.humanReadableEnvironmentString(report.environment) + " FAILED (" + report.numFailed + " out of " + report.numTests + " " + utils.pluralize("tests", report.numTests) + " failed)")
